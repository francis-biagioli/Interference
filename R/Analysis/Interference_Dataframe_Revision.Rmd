...
FORAGE_CURVES <- read_csv("~/R/FoRAGE_db_V5_Dec_20_2024_original_curves.csv")
FORAGE_CURVES <- as.data.frame(FORAGE_CURVES)

FORAGE_META <- read_csv("~/R/FoRAGE_V5_sources_and_meta_inter.csv")
FORAGE_META <- as.data.frame(FORAGE_META)

Fit_pars <- read.csv("~/Julia/Inter_fitted_parameters_Revision.csv")
...
FORAGE_META_unique <- FORAGE_META %>%
  select(
    ...
    `2D_Arena_size` = `2D_Arena size_cm2`,
    `3D_Arena_size` = `3D_arena_size_cm3`
  ) %>%
...
```{r}
write.csv(Fit_pars_meta, "FoRAGE_Inter_Revision.csv", row.names = FALSE)
```
